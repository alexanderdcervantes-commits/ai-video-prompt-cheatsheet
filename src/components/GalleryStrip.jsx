...
function GalleryCard({ item, onJump }) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 truncate px-0.5 text-xs text-mist">{item.name_en}</p>
    </div>
  );
}
    ...
    <section id="gallery" className="scroll-mt-20 border-y border-white/[0.06] bg-white/[0.015] py-12 sm:py-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="flex items-end justify-between gap-4">
          <div>
            <p className="text-[11px] font-medium uppercase tracking-[0.3em] text-gold/80">Showreel</p>
            <h2 className="mt-2 text-2xl font-bold text-white sm:text-3xl">先看效果</h2>
            <p className="mt-2 text-xs text-mist sm:text-sm">悬停即可预览 · 点击跳到这个运镜的讲解和提示词</p>
          </div>
          <a
            href="#cards"
      ...
      <div className="no-scrollbar mt-7 flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-1 sm:px-6 lg:px-[max(1.5rem,calc((100vw-80rem)/2+1.5rem))]">
        ...
        >
          ...
          <span className="text-xs">还有 {items.length - featured.length} 个</span>
        </a>
      </div>
    </section>
  );
